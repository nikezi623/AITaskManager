"""ATM data format: schema, merge, and legacy <-> state conversion.

This is the shared library for the desktop-side tools (atm_sync.py) and the
one-time migration. It mirrors docs/js/merge.js exactly; both are verified
against the same fixtures in tests/merge_cases.json.
...
REPO = HERE.parent
LOCAL_STATE_DIR = REPO / ".atm"
LOCAL_STATE_PATH = LOCAL_STATE_DIR / "atm-state.json"
...
def resolve_token(explicit: str | None = None) -> str:
    """Token from --token, $GITHUB_TOKEN, or an authenticated gh CLI."""
    if explicit:
    ...
    try:
        ...
        pass
    sys.exit("No GitHub token. Run `gh auth login`, or set GITHUB_TOKEN, or pass --token.")
...
def cmd_import(args) -> int:
    ...
    LOCAL_STATE_PATH.write_text(stable_stringify(state), encoding="utf-8")
    print(f"\nWrote {LOCAL_STATE_PATH.relative_to(REPO)}")
